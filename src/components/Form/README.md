...
|-------|---- |---------|-------------|
| modelValue? | string, number, string[], number[] | <code>none</code> | v-model |
...
| disabled? | boolena | <code>false</code> | 폼 사용불가 처리 |
| readonly? | boolena | <code>false</code> | 폼 입력 불가 처리 |
| autofocus? | boolena | <code>false</code> | 화면 로드시 커서가 자동으로 들오 오도록 할 것인지 |
