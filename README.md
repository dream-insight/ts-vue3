...
const obj = reactive<Option>({})        // Object -> 빈 오브젝트
...
// 단 object의 경우 정의된 interface에 맞게 properties를 정의해줍니다.
const obj = reactive<Options>({
  text: '',
  value: ''
})

// 또는 모든 프로퍼티를 옵셔널로 타입으로 선언 하여 사용가능하지만 지양하여야 합니다.
interface Option {
  text?: string
  value?: number
}
...
// Good
const getObjectLength = (prm?: object = {}): number => {
  return Object.keys(prm).length
}

let size: number = getArraySize()
let objSize: number = getObjectLength()
```
...
// it's okay
const okay1 = ary.map((num) => Number(intNum) + ((Number(intNum) / 2) * 0.1))
...
* 데이터가 undefined가 될 확율이 있고, API 전송이 필요한 변수에 대해서는 필히 undefined 체크를 하시기 바랍니다.
```typescript
...
```
> 정밀한 계산이 필요한 경우가 아닌경우 Int형 캐스팅은 Number() 함수를 사용하여 진행하세요.
...
* 이 두가지는 확실한 차이가 있지만, 사용자에 의해 작성된 property를 확인하는데 있어서는 별다른 차이가 없습니다.
```typescript
...
if (obj.hasOwnProperty('hasOwnProperty')) // false
```
...
// Bad
function isOpenMarket(): boolean { return isOpen }
...
// Good
const isOpenMarket = (): boolean => Boolean(isOpen())
```
...
## 9. 반복문
### 9.1. 이터러블(<code>for-of</code>) 대신 <code>map()</code>, <code>forEach()</code>와 같은 고급 함수를 사용하세요.
```typescript
...
```
> 위의 코드는 AirBnB의 JS Convention 코드를 인용하였습니다. [링크 참조](https://github.com/tipjs/typescript-style-guide#%EC%9D%B4%ED%84%B0%EB%A0%88%EC%9D%B4%ED%84%B0%EC%99%80-%EC%A0%9C%EB%84%88%EB%A0%88%EC%9D%B4%ED%84%B0iterators-and-generators)
...
import type { Increase } from '../increase/types'

const count = inject('count') as Count
const increase = inject('increase') as Increase
</script>
